 ...
 *  true == 1
 *
 * Ветка hiwonder-meta. github.com/evkuz
 ...
 * Запускаем менеджера OpenCV - http://192.168.1.175:5001
 *
 ...
 *
 * попробуем передать указатель напрямую из QSocketThread::onReadyRead()
 * ...
 * Да, так работает. " Q_ARG(QObject*, this)); "
 *
 * Работаем над
 * "(process:119673): GLib-ERROR **: 12:30:02.314: Creating pipes for GWakeup: Too many open files
 * Trace/breakpoint trap (core dumped)
 * "
 * ...
 * Интересно, что сбой происходит при "New Incoming" == 1013/1014
 * [Это из-за Qt::DirectConnection, Все исправилось после Qt::QueuedConnection]
 *
 * Есть мысль попробовать запускать "if (readSocket.GetState(&state) == 0) "
 * в отдельном потоке. Тогда переменные state.objectX, state.objectY будут постоянно обновляться.
 * ...
 * А еще надо попробвать все же EventLoop потока замутить.
 *
 * Ф-ция "QAbstractEventDispatcher *QThread::eventDispatcher() const" вызывается после запуска потока и
 * возвращает нулевой указатель, что означает, что НЕТ QEventLoop.
 * А значит смело создаем его руками.
 * ...
 * Получается не то. Крутится в цикле и события не обрабатывает.
 *
 * https://wiki.qt.io/Threads_Events_QObjects
 *
 * "Отвал башки" возникает только при вызове "start" и случается в момент вызова детекции. Т.е. это времязатратная опреация при скоростном опросе,
 * 50раз/с.
 * Попробуем, в случае команды "start", останавливать прием соединений через вызов QTcpServer::pauseAccepting() и возобновлять
 * прием соединений через QTcpServer::resumeAccepting().
 * Разницу по времени между pause и resume сделаем, скажем 20мс.
 * ...
 * Пауза в приеме соединение по таймеру не помогла, "отвал башки"
 * ...
 * И очень вероятно, что "отвал башки" происходит из-за клиента ! Т.к. клиент не дождавшись быстрого ответа закрывает сокет.
 * А прога пытается в этот сокет данные отправить.
 * ...
 * Разобраться с механизмом QTcpServer::pauseAccepting() /resumeAccepting()
 * Пока просто перестает принимать соединения, но не возобновляет.
 * Убрал. Фиксим.
 *
 *
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++++++
 * 22.08.2022
 * Настравиваем передачу
 * MainProcess -> QTcpSocket.
 * ОДнако, у нас же tcpthread->moveToThread(thread_A);
 * И какой теперь адрес у tcpthread ??? [Нам не важно, используем this->sender() для получения указателя
 * на объект, вызвавший данный слот]
 *
 * Задача :
 * - Проследить, чтобы вызов qDebug() был только из объекта QTcpSocket.[Сделано]
 *   Оставил в MainProcess в 1 месте, при открытии SerialPort-а. Это в самом начале,
 *   на дальнейшую работу в потоках не повлияет.
 * - Убрать из объекта MainProcess все вызовы qDebug(), есл есть.
 * - Изменить ф-цию MainProcess::Data_From_TcpClient_Slot(QString message, qintptr socketNumber)
 *   Вторым аргументом будет указатель на объект QSocketThread...
 *   или можно получить указатель на источник вызова через sender() ? [Да, можно. Так и сделано]
 *   ...
 *   Сделано, но не работает. Возвращаемся в эту точку.
 *   ...
 *   Аргументы ф-ции MainProcess::Data_From_TcpClient_Slot не поменялись.
 *
 * В режиме опроса в 1 сек. Прога вылетает на счетчике "New Incoming 1014"
 * (process:2285924): GLib-ERROR **: 16:35:01.915: Creating pipes for GWakeup: Too many open files
 * Trace/breakpoint trap (core dumped)
 * Сокеты/потоки не удаляются ???
 *
 * Пока под вопросом, нужно ли класс JsonInfo:: делать наследником QObject ???
 * До сих пор работало и так.
 *
 * Теперь у нас изменились библиотечные файлы ${EXTLIBS}, с другой стороны, и ветка поменялась.
 * НА данный момент папка ${EXTLIBS} не связана с git, поэтому её содержимое не меняется со сменой веток.
 * Т.е. вопрос - добавлять ли в git слежение за файлами ${EXTLIBS} ? Тогда при смене веток, они будут также меняться.
 *
 * Надо оно нам ???
 * ...
 * Пробуем. Папку ~/iqr_lit/qtlibs делаем, например ${LOCLIBS}, и прописываем в проект вместо ${EXTLIBS}
 * Тогда при смене веток они менятются.
 *
 * Пока работаем с веткой hiwonder-meta до выяснения проблем с "Too many open files"
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++++++
 * 19.08.2022
 * Пробуем QMetaObject::invokeMethod
 ...
 * Адрес (указатель) основного потока передавать при создании QTcpSocket чтобы вызвать MainProcess::Data_From_TcpClient_Slot(QString message, qintptr socketNumber)
 * через QMetaObject::invokeMethod
 * ...
 * Заработало !!!
 * Передача данных QTcpSocket -> MainProcess работает.
 * Теперь надо настроить обратную передачу, т.е. MainProcess -> QTcpSocket.
 *
 * Интересно, что слот Data_From_TcpClient_Slot(QString, qintptr) заработал и без макроса Q_INVOKABLE
 *
 * И хорошо бы сравнить скорость взаимодействия через QMetaObject::invokeMethod и через signal/slot механизм.
 * По уму надо новую ветку создавать.
 * Наверное лучше отдельный проект в той же ветке, типа hiwonder-web-meta :)
 *
 * Теперь из supervisor-а не срабатывает команда "put_box", однако из браузера и через gui-client все работает как надо.
 *
 * Также надо следить за файлами из $${EXTLIB} чтобы для разных веток был свой функционал и код.
 *
 * Важно !
 * Для удобства передачи указателя QObject* теперь MAinProcess запускается через :
 * QObject* ptrMainProcess = new MainProcess();
 * А не через
 * QSimpleServer server;
 *
 * Помогло наличие ветки "release", а в ней рабочий каркас проекта SimpleServer.
 * Через этот проект потестировал.
 *
 * Появилась ошибка :
 * (process:1139097): GLib-ERROR **: 16:32:30.956: Creating pipes for GWakeup: Too many open files
 * Перешел на ветку hiwonder-meta
 *
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++++++
 * 16.08.2022
 * Проверял дублирование "start" - да, дублируется. Т.е. после прихода команды "start" менее чем через 1с. приходит еще одна такая же.
 * Это приходит из супервизора, так, что реагируем в соответствии с протоколом.
 * QSimpleServer ловит обе пришедшие команды, отправляет их "наверх" в слот MainProcess::Data_From_TcpClient_Slot(QString message, qintptr socketNumber).
 * Робот(слот), однако, отвечает только на первую пришедшую, в соответствии с протоколом.
 * При этом соответствующее сообщение - "This action is STILL running" - в лог не пишется для ускорения обработки запроса.
 * ОДнако клиенту такое сообщение в качестве ответа -  отправляется.
 * Проблема только с командой старт. Команда "put_box" не дублируется, все норм.
 *
 * //++++++++++++++++++++++++++++++++++++++++++++++++++
 * 15.08.2022
 * Большой прогресс, ловми favicon :)
 * После выключения опции :
 * settings -> Privacy and security -> Cookies and other site data -> Preload pages for faster browsing and searching -> switch OFF
 * Осталось его (сам файл) отправлять, чтобы Chrome перестал спрашивать.
 *
 * Самое важное, что теперь "GET /favicon.ico HTTP/1.1" приходит отдельным сообщением, виден в логе.
 * Также важно, что запрос favicon приходит ТОЛЬКО ПРИ РАЗОВОМ ЗАПРОСЕ вручную из браузера.
 * Супервизор не посылает запрос на favicon !
 * По крайней мере, процесс robot.hiwonder.service - не посылает.
 * supervisor - тоже не посылает, проверено.
 *
 *
 * //++++++++++++++++++++++++++++++++++++++++++++++++++
 * 11.08.2022
 ...
 * 15.08.2022
 * Выяснил.
 * На это влияет настройка самого CHROME. Её надо выключить.
 *
 * settings -> Privacy and security -> Cookies and other site data -> Preload pages for faster browsing and searching -> switch OFF
 *
 *
 ...
 * 31.07.2022
 * ВАЖНО !
 * Сейчас ПО написано с расчетом, что только 1 Serial-USB устройство подключено к ПК и это есть наш робот.
 * Другими словами, сейчас устройства перебираются просто в порядке возрастания, кто откроется - тот и есть наш робот.
 * Однако, если к ПК подключено более 1 Serial-USB устройства (RPLIDAR, JETSON XAVIER  и т.д), то может так получиться, что
 * соседнее устройство получит адрес ttyUSB0 - т.е. более ранний в списке по возрастанию, а наш робот - ttyUSB1.
 * Тогда ПО будет оптравлять данные в другое устройство, ожидая ответа от hiwonder.
 * В общем, следующая задача :
 * - написать процедуру идентификации девайса - handshake - чтобы девайс давал правильный ответ на запрос,
 *   скажем, "hello, HiWonder". И если ответ неправильный - перебирать дальше устройства по списку Serial Device-ов.
 *
 * Также материал для статьи :
 * Браузер посылает GET-запрос 2-мя посылками сразу, одна за другой. А gui-client - одной посылкой.
 * При этом сервер на ПК(архитектура Intel/AMD-x86-64) это воспринимает как 1 команду в обоих случаях.
 * Однако, сервер на Raspberry Pi 4B (архитектура ARM-x64) ИНОГДА воспринимает это как 2 команды, или даже 3 в случае посылки из браузера.
 * И как 1 посылка, если из gui-client.
 *
 * //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 * 18.07.2022
