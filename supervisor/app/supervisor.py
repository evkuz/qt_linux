...
class Supervisor(object):
    ...
    def _initialize(self):
        ...
        #self._devices['wheeltec'] = agents.Wheeltec("http://159.93.69.229:5001", self._updateInterval)
    ...
    def run_action(self, device:str, action:str)->dict:
        if not device in self._devices:
            return {'rc' : -9, 'info' : "No device was found!"}
        if self._devices[device].run_action(action):
    ...
    def get_info(self, device:str, service:str, **kwargs)->dict:
        if not device in self._devices:
            return {'rc' : -9, 'info' : "No device was found!"}
        return self._devices[device].get_info(service, **kwargs)