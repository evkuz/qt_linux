...
{
    connect(&serial, &QSerialPort::readyRead, this, &SerialDevice::ReadFromSerial_Slot);
}
...
void SerialDevice::ReadFromSerial_Slot()
{
    ...
    qint64 numBytes, sz;
        ...
        qbuf = serial.readAll(); // У нас тут щас hex-данные

        sz = qbuf.size();
        str2 = "";
        bool ok;
        for (int i=0; i < sz; i++){
            // Так получаем на экране hex-данные в строковом виде, НО БЕЗ префикса 0x
//            str2 += QString::number(static_cast<unsigned char>(qbuf.at(i)), 16);//QString("0x%1: ").arg(value, 2, 16, QChar('0'));
            // Выводим данные в hex-виде
            //str2 += "0x";
            //str2 += QString::number(static_cast<unsigned char>(qbuf.at(i)), 16);
            //char a = static_cast<char>(qbuf.at(i));
            unsigned char a = qbuf.at(i); //(uchar(qbuf.at(i))
            str2 += QString("0x%1").arg(a, 2, 16, QChar('0'));
            str2 += ", ";
        }
        // Убираем последние  ", "
        str2 = str2.remove(str2.size()-2,2);
        // По идее на этом всё. Далее emit signal (GetData_Signal) от том, что данные в массиве готовы к обработке.
        str = "From device : ";
        str += str2; //QString(qbuf.toHex());
        // Выводим значение value как 0xзначение
        //str2 = QString("0x%1: ").arg(value, 4, 16, QChar('0'));
