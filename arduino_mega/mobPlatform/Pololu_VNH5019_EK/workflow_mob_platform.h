 ...
 * //+++++++++++++++++++++++++++++++++++++++++++++++
 * 30.01.2023
 * Настраиваем коэффициенты ПИД. Так драматично скорость не должна расти.
 * Где-то коэффициенты имеют чрезмерную величину.
 * 
 * Часто значение deltaT получается отрицательное... 
 * deltaT - интервал времени в секундах между вызовом ф-ции обработчика прерываний таймера.
 * Нормальное значение при текущих настройках около 0.9с.
 * Откуда берется отрицательное значение времени? (это разница между текущим и прошлым временем)
 * //+++++++++++++++++++++++++++++++++++++++++++++++
 * 26.01.2023
 * ПИД запускается внутри обработчика таймера, независимо от команды.
 * ТАк у нас скорость колес постоянно корректируется.
 *
 * Теперь по команде "start" обнуляются все счетчики.
 *
 * ПИД запускается, если разница в энкодерах больше 50 и после этого скорость отстающего увеличивается сразу на 80, т.е. почти удваивается
 * Меняем флаг leaderIsChanged на isSomeOneLeading, чтобы код был понятнее.
 *
 * Теперь надо подобрать коэффициенты, чтобы скорость возрастала пропорционально разнице в показаниях энкодеров. 
 * В целом, ведет себя правильно, нужно сделать все плавнее.
 * После выравнивания показателей и сброса скорости, все равно увеличивается скорость по ПИД.
data: "posAm1=5536, posAm2=5568, "
---
data: "posAm1=6430, posAm2=6210, "
---
data: "float Kp*E=12.5000, PID(u)==newSpeedLag = 82"
---
data: "m1Speed 100, m2Speed 182"
 Т.е отстающее колесо уже обогнало, а скорости у них при этом не равны...
 Надо убрать encodersGAPdelta
 * А как тебе такое, Илон Маск ?!
 *
 * data: "posAm1=962, posAm2=1039, "
---
data: "float Kp*E=-2.9231, PID(u)==newSpeedLag = 3341"
---
data: "m1Speed 3441, m2Speed 100"
---
data: "!!! Speed values aligned !!!"
