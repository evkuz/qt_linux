 ...
 * //++++++++++++++++++++++++++++++++++++++++
 * 16.09.2022
 * Вчерашнее ограничение сыграло злую шутку.
 * Команды, отправляемые по кнопке мобману вида
 * GET /run?cmd=ready HTTP/1.1\r\nHost: 192.168.1.201:8383\r\nAccept: ...
 * получают ответ от мобмана размером МЕНЬШЕ 200 байт, например
 * "HTTP/1.1 200 OK\r\ncontent-type: application/json\r\nAccess-Control-Allow-Origin: *\r\n\r\n{\n    \"info\": \"Another action is already runni
ng\",\n    \"name\": \"ready\",\n    \"rc\": -2\n}\n"
The packet size in bytes is  "170" (тут учитываем, что \r, или \n - это 1 символ в hex-формате.
 * Соответственно, gui-client в слоте MainWindow::onDEVSocketReadyRead_Slot() такие сообщения не обрабатывает,
 * как-будто вообще данных не было.
 *
 * Например, ответ на запуск команды "ready" от мобмана приходит в виде
 *
 * "HTTP/1.1 200 OK\r\ncontent-type: application/json\r\nAccess-Control-Allow-Origin: *\r\n\r\n{\n    \"info\": \"TEST position\",\n    \"name\": \"ready\",\n    \"rc\": 0,\n    \"state\": \"inprogress\"\n}\n"
 * The packet size in bytes is  "176"
 * А т.к. 176 < 200, то слот никак не реагирует.
 *
 * В общем, уменьшил порог до 100.
 *
 * //++++++++++++++++++++++++++++++++++++++++
 * 15.09.2022
 * Убрал обработку сигнала &QTcpSocket::stateChanged
 * На данном этапе это уже не нужно.
 * Так меньше сообщений в логе, быстрее находим нужную инфу.
 *
 * Добавил в слот чтения данных из сокета MainWindow::onDEVSocketReadyRead_Slot()
 * минимальный порог размера сообщения в 200 байт, чтобы ответ от CV камеры приходил не в виде
 * 3-х сообщений :
 * - "HTTP/1.0 200 OK" == 17  байт
 * - HTTP Заголовки    == 161 байт
 * - Само сообщение в JSON-формате. == 236 байт // А может и меньше, например, 208 байт.
 * Так слот запускается не 3 раза на кажду малую порцию, а 1 раз, когда в буфере накопится хотябы
 * 200 байт.
 *
 * //+++++++++++++++++++++++++++++++++++++++
 * 08.09.2022
 * Добавил 2 кнопки для команд отключения/подключения сервоприводов от пинов платы Ардуино.
 * Команды detach/attach соответственно.
 * Так экономится энергия аккумулятора пока приводы не используются.
 * А также нет этого режущего слух шума.
 *
 * Надо что-то делать с выбором через GUI ip, куда будет отправляться команда по tcp.
 *
