...
/*
 * Ветка mobman-floor. Кубик на полу, другой набор точек. Разработка для мобильного робота (мобильный манипулятор)
 * Перешел обратно с ветки mobman-light, чтобы больше не путаться между ними (mobman и mobman-light).
 * Работаем пока в одной ветке - mobman.
 ...
 *   т.е. весь файл processAction.cpp (260 строк) - вот поэтому ветка "-light"
 *
 ...
 * Также отличается путь к этим файлам.
 *
 * /home/ubuntu/iqr_lit/qtlibs/manipulator/meta
 *
 ...
 *
 * Многоточие "..." означает "прошло сколько-то времени и", либо, "мы подумали и"
 *
 ...
 * ...
 * Уже парсинг по наличию json-ключа "detected", "distance"
 ...
 * 29.03.2023
 * Чистим код.
 * Выплняем перегрузу слота QSocketThread::Data_2_TcpClient_Slot
 * Их теперь будет 2.
 * Слот QSocketThread::Data_2_TcpClient_Slot(QString data, qintptr socketNumber)
 *
 * использует параметр "qintptr socketNumber" для отладки  потоков.
 * Был крайне полезен для ловли "подмены" номера потока из-за переписывания некоего указателя в другом слоте.
 * При штатной работе не используется.
 *
 * Слот void Data_2_TcpClient_Slot(QString data)
 * Используется для штатной работы робота. Лишних параметров не имеет.
 *
 * //+++++++++++++++++++++++++++++++++++++=
 * Надо проверить работу QJsonArray Class.
 * "A JSON array is a list of values." В нашем случае, каждое из values есть QJsonObject.
 * Нам может облегчить задачу то, что "A QJsonArray can be converted to and from a QVariantList."
 *
 *
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++
 * 21.11.2022
 * В ответе на запрос статуса экшена вмест (-4) ставим 0.{"result" : 0}
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++
 * 18.11.2022
 * Задача : [сделано]
 * - Проверить, можно ли упростить код ф-ции JsonInfo::setActionData(QJsonObject &theObj)
 *   т.к. сейчас обновление состояния экшена довольно громоздко.
 *   Операция перезаписи данных в масcиве QJsonArray, каждый элемент которого это QJsonObject
 *   выполняется как следующие шаги
 *   - Получаем аргумент QJsonObject
 *
 *   - Ищем в нашем массиве элемент QJsonObject, у которого "name" совпадает с "name" у аргумента.
 *   - Переписываем элемент в массиве значением аргумента
 *     При этом, если вызов JsonInfo::setActionData пришел из
 *     MainProcess::ActionLaunch_From_TcpClient(), то в данном случае в аргументе у
 *     JsonInfo::setActionData не будет поля "info", и, поэтому, когда произойдет перезапись элемента массива
 *     QJsonArray в новом элементе массива также не будет поля "info". И если в этот момент вызвать команду
 *     /status?action=<action name>, то в поле "info" будет пустота, т.е. {..."info": "",...}.
 *     Однако, в дальнейшем, если вызов JsonInfo::setActionData прийдет из MainProcess::returnActionLaunch(QJsonObject &theObj),
 *     то значение поля "info" восстановится.
 *
 *   - ЧТобы избежать вышеописанных коллизий, переписывание массива QJsonArray в ф-ции JsonInfo::setActionData(QJsonObject &theObj)
 *     чуть усложнено.
 *
 *   ...
 *   Исходим из того, что поле "info" для каждого элемента массива "QJsonArray  jsnObjArray" - статично, нигде не меняется.
 *   Меняются только поля "state", "reasult"
 *   - Получаем аргумент QJsonObject
 *   - Ищем в нашем массиве элемент QJsonObject, у которого "name" совпадает с "name" у аргумента.
 *   - Сохраняем в переменной "QString info" значение поля "info" у найденного элемента массива, который потом перепишем.
 *   - Переписываем элемент в массиве значением аргумента QJsonObject
 *   - Сохраняем во временном "QJsonObject temp" уже переписанный элемент массива
 *   - Переписываем поле temp["info"] значением, сохраннённым в переменной "QString info"
 *   - Переписываем элемент в массиве значением временного "QJsonObject temp", с обновлённым полем "info"
 * ...
 * Можно упростить JsonInfo::setActionData(QJsonObject &theObj)
 * - Получаем аргумент "QJsonObject &theObj"
 * - Ищем в нашем массиве элемент QJsonObject, у которого "name" совпадает с "name" у аргумента.
 * - Сохраняем в переменной "QString info" значение поля "info" у найденного ЭЛЕМЕНТА МАССИВА, который потом перепишем.
 * - Переписываем поле ["info"] АРГУМЕНТА(theObj["info"] ) значением, сохраннённым в переменной "QString info"
 * - Переписываем ЭЛЕМЕНТ МАССИВА значением АРГУМЕНТА "theObj", с обновлённым полем "info"
 *
 *
 *
 *
 * - Команда "get_box" теперь приходит с дистанцией :
 *   action?name=get_box&distance=235
 * - Поправил границы дистанции, в которых работаем.
 *
 *
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++
 ...
 * Решаем задачи, поставленные вчера вечером.
 * - Сделано
 * Проблема была из-за того, что в методе
 ...
 * "get_box" - команду приняли - state == init
 * Если детекция есть - state == run
 * Если детекции нет - state == fail
 *
 * Внутри "action_list": [] Поле "info" связано с "result", но не с "rc", и есть единственный
 * источник инфы о состоянии экшена.
 * В заголовке ответа на запрос статуса, т.е. перед "action_list": []
 *
 * - Перелопатил ф-цию MainProcess::returnActionLaunch(QJsonObject &theObj)
 * Теперь нет параметра QObject* theSender.
 *
 * Задача :
 * - Доработать блок кода if (matched){} ф-ции QSocketThread::onReadyRead()
 *   Так, чтобы парсить всю строку, например
 *   /status[?param=value[&param2=value2]&]
 *   где param=="action"
 *   value==<action name>, например, "get_box"
 *
 ...
 * 15.11.2022
 * Решаем задачи, поставленные вчера вечером.
 * - Возвращаем deleteLater() в QSocketThread::Data_2_TcpClient_Slot
 *  ...
 *  И все нормально заработало.
 *  ОСтавил только вызов сигнала emit stopThread_signal();
 *  По этому сигналу объект QThread делает quit(), а объект QSocketThread - deleteLater()
 * - Убираем QSocketThread::theLoop() <---> this->toBeClosed, чтоб не смущало.[сделано]
 * ...
 * Есть мысль запрос  "isAttached?" перевести в разряд сервисов, получаемых по команде /service?name=isAttached
 * где
 * - rc==0 - "Yes, attached"
 * - rc==1 - "No, not attached"
 * ...
 * Сейчас экшены attach, detach переходят в состояние "inprogress".
 * НО по текущей логике кода эти экшены должны перейти в состояние "done" после срабатывания слота
 * MainProcess::Moving_Done_Slot(), именно там происходит эта смена.
 * А слот срабатывает, если из serial port пришло сообщение, содержащее строку "LAST".
 * Вот это не было сделано для команд attach/detach.[сделано]
 * Обновил arduino, все работает как надо.
 *
 * Сейчас нет ответа на запрос "/service?name=getservices", только на "/service?name=getactions"
 * Поэтому задача :
 * - сделать обработку запроса сервисов по аналогии с getatcions, т.е. через switch(){}, чтобы в дальнейшем
 *   было удобно добавлять сервисы.[сделано]
 *   Доработана ф-ция JsonInfo::init_actions_new()
 *
 * ...
 * Убрана ф-ция void init_json() как неиспользуемая. 309 строк кода.
 * Пока закомментировал, в следующем коммите удалю.
 * - ф-ция void init_json() удалена.
 *
 * Есть проблема с ответом в ситуации, когда выполняется экшен, а в это время приходит запрос
 * на запуск экшена.
 * Ставим в задачу на завтра.
 * p.s. Проблема была из-за того, что в методе
 * Q_INVOKABLE void Data_2_TcpClient_Slot(QString data, qintptr socketNumber);
 * Не отправлялся параметр qintptr socketNumber
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++
 * 14.11.2022
 * Переделываем ф-цию QSocketThread::onReadyRead() под 3 разных типа запроса.
 * В обоих случаях, -  "/run?cmd=" и "/status", - вызываем "StatusRequest_From_TcpClient"
 * В дальнейшем уберем "/run?cmd=".
 * ...
 * Как водится, была проблема с передачей указателей и переменных в новые ф-ции, назовём это "lost in transferring"
 * ...
 * Заработало.
 * ...
 * Есть проблема с потоками.
 * QThread::start: Thread creation error (Resource temporarily unavailable)
 * Думаю, банально не все корректно удаляются, а поэтому накапливаются.
 * ...
 * Также задача : [Ф-ция QSocketThread::onReadyRead()]
 *
 * - Разобраться с if-else для favicon
 * - Разобраться с if-else для matched
 *
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++
 * 11.11.2022
 * Вскрылась новая бага.
 * При отправке ответа тср-клиенту происходит закрытие сокета.
 * И это работало.
 * Но настал момент, когда сервер дает ответ клиенту за 2 захода: сначала быстрый ответ,
 * а затем содержательная часть. Например при запросе "getactions" нужно дать быстрый ответ, что запрос принят.
 * А потом еще и ответ на сам запрос отправить, в данном случае это не статус.
 * ...
 * Не совсем так.
 * get_actions - это, согласно протокола, запрос информации из сервиса
 * Тогда ближайшее решение - создать 2 функции ответа тср-клиенту, с обрывом сокета и без(т.е. с сохранением)
 * ...
 * Ага, запрос "getactions" - это все же на запуск экшена, а запрос сервиса.
 * Тогда модернизируем парсинг, различая 3 вида tcp-запросов :
 * - статус
 * - запуск экшена
 * - информация из service
 *
 * Все это требует существенной переделки кода.
 * ...
 * Т.к. ответы разные, и много экшенов, то если готовить ответ в одной ф-ции, то будет много дублирования кода, как минимум,
 * для многих экшенов, а если еще и сервисов разных будет больше 2, то универсальная, т.е. одна ф-ция ответа на все
 * типы запросов будет громоздкая, с большим дублированием кода.
 * Поэтому пока, на сегодня, разделим обработку запроса от тср-клиента на 3 разные ф-ции, по одной
 * на каждый тип запроса.
 * А это, блин, много кода писать и отлаживать...
 * Начнем добавлять по одному.
 *
 * Теперь ВСЕ ответы на запросы, - одна посылка в сокет. (!)
 *
 * Но запрос на формирование ответа - в разные ф-ции.

* Ответ на запрос статуса:
 * - MainProcess::StatusRequest_From_TcpClient(QObject* theSender)
 *
 * Ответ на запуск экшена:
 * - MainProcess::ActionLaunch_From_TcpClient(QObject *theSender, QString actionName)
 *
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++
 * 10.11.2022
 * В режиме "на вису" была добавлена мат. обработка-корректировка массива точек на основе дистанции.
 * В результате такой обработки вычисляется новый набор значений приводов, скорректированных пропорционально
 * разнице в значениях дистанции - заранее набранной и фактической.
 * Теперь эту обработку переносим на режим "на полу".
 * В этом режиме для каждой точки свой массив, и таких массивов набрано 12шт. Теперь же создадим 2-мерный массив,
 * по аналогии с режимом "на вису", где первый элемент строки - значение дистации в мм, а 2й, 3Й элементы - значения
 * приводов 3,4. Значения приводов 1,2 всегда {35, 90} - хват открыт, расположен горизонтально.
 * ...
 * Вводим новую команду getRange, которая в ответ отправляет диапазон расстояний детекции, в котором манипулятор в состоянии
 * взять кубик.
 * //+++++++++++++++++++++++++++++++++++++++++++++
 * 10.11.2022
 * Возвращаемся к режиму работы mobman, когда кубик на полу(а не не вису).
 * Т.к. в таком режиме совсем другие точки с положениями приводов, то далее ведем проект в новой ветке
 * mobman-floor
 * Следует удалить ветку mobman-light после всех тестов работы в этой ветке.
 * Проверить, что не потерялись вспомогательные файлы - схемы, описания и т.д.
 * servo_data.ods
 * mobman-soft.dia
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++
 * 20.10.2022
 * Набрали 10 точек - позиций захвата мобмана в диапазоне distance=[147, 249]
 * Точки сохранены в отдельном файле servo_data.ods
 * Также сдублированы в файл "protocol.h" в данном проекте.
 * Нарисовалась следующая задача :
 *
 * - разработать и реализовать алгоритм пересчета текущей позиции мобмана в значения углов серво на основе набранных 10 точек.
 *   Естественно, "текущая позиция мобмана" должна быть в диапазоне набранных точек distance=[147, 249]
>>>>>>> mobman/demo01/evkuz.h
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++
 * 19.09.2022
 * Сделать вывод команды "getactions" не по алфаиту json-ключей, а по порядку возникновения оных в объекте.
 * Вот тут как раз может пригодиться библиотека hlohmann
 * ...
 * Получили почти то, что нужно.
 * Надо разобраться с header["action_list"] = {};
 * Текущая версия 3.10.4, тогда как вышла уже версия 3.11.2(Release date: 2022-08-12).
 * ОБновляем.
 * ...
 * Вот после обновления версии всё выводится красиво.
 * Т.е. список ["action_list"] - если пустой - не выводится вообще.
 * ...
 * Сделал, чтобы пустой список ["action_list"] - т.е. когда нет активных экшенов,
 * и робот ждет команду, - выводится  как  "action_list": []
 * Добавил ф-цию JsonInfo::init_actions_new(). В ней инициализация статусов экшенов для вывода
 * по команде "getactions"
 * Фиксим.
 * Задача :
 * - Добавить экшен "setservos=".
 * Уже он есть, но парсер находит только среди неизвестных команд, т.к. его нет в списке
 * QSocketThread::strcommand
 * Задача подразумевает прописывание статусов, параметров и т.д.
 * ...
 * [Сделано]
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++
 * 16.09.2022
 * Команда "reset" все время в состоянии  {"state": "inprogress"}, соответственно,
 * другие команды не выполняются.
 *
 * Тут проблема в том, что команды, отправляемые из gui-client, почти все умещаются в посылку меньше 200 байт.
 * При этом мобман их выполняет, но клиенту ответ не посылает.
 * На самом деле посылает, только размер посылки очень часто менее 200 байт, а клиент не принимает такие в обработку.
 *
 * Задача:
 * - Поправить вывод имени девайса. Сейчас все время выводится "HiWonder", даже если это mobman.
 *   Сейчас имя девайса задается в файле jsoninfo.h :
 *   #define DEV_NAME "HIWONDER"
 *   Надо продумать, чтобы файл jsoninfo.h был максимально универсальным.
 *   Будем задавать имя девайса в конструкторе JsonInfo::JsonInfo() при создании объекта.
 *   А еще лучше передавать это имя в конструктор при создании объекта.
 *   ...
 *   [сделано] Имя девайса передается в конструктор в качестве единственного параметра, при создании объекта.
 *
 * - Убрал команду "clamp" за ненадобностью, заменил на две -  "lock", "unlock".
 *   В массиве tcpcommand её сохранил, чтобы индексы команд не сдвинулись.
 *   Убрал из массива JsonInfo::jsnObjArray. Все работает, проверено.
 *   В списке "getactions" теперь не выводится.
 * //+++++++++++++++++++++++++++++++++++++++++++++
 * 15.09.2022
 * Убрал отладочную печать с данными от CV из файла MainProcess::CV_NEW_onReadyRead_Slot()
 * Еще можно подчистить вывод в консоль через qDebug() из класаа QSocketThread, т.к. её все равно не видно, если
 * работ запускается через systemd.
 *
 * Переименовал класс HiWonder -> SerialRobot.
 * Т.к. данный класс используется и для HiWonder и для mobman.
 * Вполне возможно подключить класс serialRobot в EXTLIBS/LOCLIBS.
 *
 * Ветку mobman-light (без processAction, но с metaObject вызовами) загружаю в git.jinr.ru, там смёрджу с mobman.
 * ...
 * Ещё точек надо набрать...
 *
 * Решена задача от 09.09.2022
 * - Адаптировать код слота MainProcess::CV_NEW_onReadyRead_Slot(), чтобы слот начинал обработку данных, когда
 *   придет ВСЁ сообщение от CV, т.е. это где-то...414 байт
 *   т.к. сейчас ответ от CV идет 3-мя пачками :
 *   - "HTTP/1.0 200 OK" == 17  байт
 *   - HTTP Заголовки    == 161 байт
 *   - Само сообщение в JSON-формате. == 236 байт // А может и меньше, например, 208 байт.
 *   Итого получаем 414 байт, однако условие поставлено if (befbytes < 200) {граница в 200 выше 177 байт, (17+161)>177}
 *   помог gui-client, вывод лога сообщил все необходимое.
 *
 * Все это, конечно, не может претендовать на универсальное решение. А может это и +, т.к. сервер можно настроить
 * на каждый конкретный случай. Посмотрим, как это сделано в python и тогда можно будет делать выводы.
 *
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++
 * 14.09.2022
 ...
 * //+++++++++++++++++++++++++++++++++++++++++++++
 * 13.09.2022
 * Переходим на ветку mobman-light, где не будет ф-ции. processAction
 * Пока делаем фикс.
 *
 *
 * //++++++++++++++++++++++++++++++++++++++++++++
 * 10.09.2022
 * Меняем DirectConnection...
 * ...
 * Работает
 *
 *
 ...
 * //++++++++++++++++++++++++++++++++++++++++++++
 * 09.09.2022
 * Близко к тому, чтобы обойтись БЕЗ ф-ции processAction. Возможно, исторически, было удобнее взять из hiwonder
 * этот код и адаптировать к мобману...
 * ...
 * Еще задача : [сделано 15.09.2022]
 * Адаптировать код слота MainProcess::CV_NEW_onReadyRead_Slot(), чтобы слот начинал обработку данных, когда
 * придет все сообщение от CV, т.е. это где-то ...414 байт
 * т.к. сейчас ответ от CV идет 3-мя пачками :
 * - "HTTP/1.0 200 OK" == 17  байт
 * - HTTP Заголовки    == 161 байт
 * - Само сообщение в JSON-формате. == 236 байт
 * Т.е. нужно посчитать размер (17+заголовки) -> переменная okAndHeaders==178 ->  байт и обрабатывать
 * сообщение при превышении размера okAndHeaders.
 * Так будет 3 срабатывания слота и 1 обработка, а сейчас 3 на 3.
 * ...
 *
 * Выяснить время (в мс), за которое приходят все три порции сообщения, это где-то 2-3-5мс ?
 *
 * Привести вывод getactions в соответствие с протоколом.
 * Требуется преобразование полей.
 * info -> description
 * state -> statuses
 * rc - убираем
 * parameters - добавляем. (setservos сюда попадает)
 * ...
 * Закоментил отладочный вывод в MainProcess::Moving_Done_Slot() для ускорения работы.
 * Надо будет проверить, чтобы сервер был отдельным хостом.
 *
 * //++++++++++++++++++++++++++++++++++++++++++++
 * 08.09.2022
 ...
 * //++++++++++++++++++++++++++++++++++++++
 * 02.09.2022
 * Нужно выводить в qDebug указатель на объект, которому принадлежит ф-ция QSocketThread::Data_2_TcpClient_Slot(QString data, qintptr socketNumber)
 * Так сможем сравнить адреса объектов в
 * MainProcess::CV_NEW_onReadyRead_Slot() и QSocketThread::Data_2_TcpClient_Slot(QString data, qintptr socketNumber)
 * ...
 * Выяснилось, что при срабатывании сигнала &QIODevice::readyRead и последующем вызове слота &MainProcess::CV_NEW_onReadyRead_Slot
 * "теряется" значение указателя QObject *ptrTcpClient, т.е. :
 * На момент использования значения этого указателя там уже другой адрес - адрес вновь пришедшего запроса (сейчас это status)
 * Поэтому, для сохранения адреса объекта, вызвавшего запрос к CV через сокет в ф-ции ,  - сделано
 * повторное копирование указателя в другую переменную - QObject *newPtrTcpClient;
 * Буквально :
 * После всех connect-ов
 * this->newPtrTcpClient = this->ptrTcpClient;
 *
 * И уже в ф-ции MainProcess::CV_NEW_onReadyRead_Slot() используем значение указателя на объект из переменной
 * this->newPtrTcpClient
 * Так нет "segmentation fault".
 * Отметим, что смена QMutex mutex; на QRecursiveMutex mutex; не помогла.
 * Сработало только копирование указателя перед выходом из ф-ции MainProcess::request_CV()

 * ...
 * Конечно, это костыль. Но сейчас, главное, чтоб работало.
 * Со временем разберусь. Нужно исопльзование smartPointer, sharedPointer и т.д.
 * ...
 * Также пока только 7 раз подряд без сбоев сработало взятие кубика (а надо минимум 10).
 * Т.е. программа остается в рабочем состоянии, но статус все время "inprogress" и "reset" помогает.
 * Тут, скорее всего, где-то проглатывается обновление jsnObj
 * Фиксим.
 *
 * Задача :
 * Вынуть блок  if (this->DETECTED != true){...} из ф-ции parseJSON
 *
 * Подумать как быть с разницей в коде для Raspberry pi и для HiWonder.
 *
 *
 *
 * //++++++++++++++++++++++++++++++++++++++
 * 30.08.2022
 * ДОбавил инициализацию и ресайзинг QByteArray массивов в классе QSocketThread
 *
 * //++++++++++++++++++++++++++++++++++++++
 * 25.08.2022
 * Указатель на QJsonObject::mainjsnObj передается для "put_box", но не передается для "get_box"
 * Поправил. Иногда clean project не все чистит...
 * Множество отладочной печати теперь нужно убрать.
 * Вывод serial port в отдельный файл.
 *
 * Ошибка возникает когда нет детекции. Неправильно пишутся статусы
 * ...
 * jsnStore->isAnyActionRunning
 * ...
 * No detection vs distance out of range
 *
 *
 * //++++++++++++++++++++++++++++++++++++++
 * 24.08.2022
 * Переходим на ветку mobman-meta
 * классы QSocketThread MainProcess обмениваются сообщениями через cигнал/слоты, либо через metaSystem
 *
 * //++++++++++++++++++++++++++++++++++++++
 * 18.08.2022
 * Ответ от CV-камеры приходит как 2 сообщения :
 * - "HTTP/1.0 200 OK"
 * - "Само сообщение"
 * Соответственно, слот ловит первое сообщение, это даже не JSON-объет, со всеми выходящими... получаем "distance out of range"
 * Второе сообщение обрабатывается правильно, НО... почему-то в Tcp-сокет не отправляется правильный ответ...
 * Поправил.
 * Фиксим.
 * ...
 * "0xf00f: The Command with index 11 is not found" - а в списке команда есть !
 * Добавляем.
 * ...
 * Добавил.
 * Теперь надо бороться с вылетанием...
 *
 *
 *
 * //++++++++++++++++++++++++++++++++++++++
 * 17.08.2022
 * Если distance out of range, то экшен все равно становится "inprogress", что в корне не верно.
 * Исправляем [в процессе].
 * Ф-ция processAction взята из проекта для робота hiwonder.
 * Там есть ссылка к индексу команды... для робота hiwonder...
 * Надо заменить значение индекса на символьное значение команды, чтоб из названия команды
 * было понятно, про что речь.
 * ...
 * Пересекаются/дублируются статусы экшенов в файле jsoninfo.h, mainprocess.h
 * Следует оставить эти данные только в файле jsoninfo.h, т.к. это будет переносимый класс из одного проекта в другой.
 * Ибо в данный момент возникает путаница, избавляемся от неё.
 *
 * JsonInfo теперь наследуем от QObject (не было, а думал, что есть)
 * class JsonInfo : public QObject
{
    Q_OBJECT
 * //++++++++++++++++++++++++++++++
 * 16.08.2022
 * Переделал ф-цию listen, теперь слушаем только для адреса "192.168.1.175".
 * Дубль от supervisor-а приходит, нужно поправить - привести в соответствие с протоколом.
 * ...
 * Упорно срабатывает отправка данных tcp-клиенту из условия  if (rDistance < 110 || rDistance > 230),
 * хотя в это условие прога не входит т.к. нет в логе об этом записи.
 ...
 * //++++++++++++++++++++++++++++++
 * 12.08.2022
 * Обнаружилась засада с ф-цией QString::indexOf.
 * В команде "unlock" встречается строка "lock" и происходит неверное определение команды.
 * Т.е. тут, в ф-ции int MainProcess::getIndexCommand(QString myCommand, QList<QString> theList),
 * нужно искать не по индексу, а по точному совпадению.
 *
 * //++++++++++++++++++++++++++++++++++++++++++++++++++
 * 11.08.2022
 * Есть мысль добавить команду "name?" в обмен по Serial Port, чтобы избежать подключению не к тому порту,
 * в случае, когда имеем несколько usb-serial устройств на одном роботе.
 *
 * Софт mobman-а не ловит запрос на favicon.ico.
 * Очень похоже, что это глюк ARM-архитектуры.
 * Проверю на ПК.
 * Пока фиксим.
 *
 * //++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 *
 * Баг :
 * Когда нет детекции и запускается get_box, то запускается и экшен, при этом сам манипулятор не двигается. Этого не должно быть.
 * Или крестик сними или ...
 * ТАкже старые проблемы :
 * 12
$$$$$$$$$  QThread(0xb3003df0)  $$$$$$$$$$ new socket $$$  "10"
13
$$$$$$$$$  QThread(0x543178)  $$$$$$$$$$ new socket $$$  "12"
thread ID  QThread(0xb3003df0) !!!!!!!!!!!!!!!!!!!!! Have got  460  bytes of Data FROM TCP SOCKET !!!!!!!!!!!!!!!!!!! "10"
!!!!!!!!!!!!!!!!!!!!! Get COMMAND FROM QSocketThread::onReadyRead(), i.e. from TCP SOCKET  "10"
************ QThread(0xb3003df0) "put_box"

Т.е. команда приходит 2-мя посылками. Пока это беда.
 *
 * //++++++++++++++++++++++
 * 21.07.2022
 * Наладить выполнение статусов для экшенов
 * - "ready"
 * - "parking"
 * - "get_box"
 *
 ...
 * //++++++++++++++++++++++++++++++++++++++++++++++++
 * 20.07.2022
 * Подключаем сюда все изменения по протоколу из проекта hiwonder.
 * Теперь все файлы по tcp-обмену подключаются из внешней библиотеки {EXTLIBS} - вот и надо подключить.
 * Меняем mainprocess т.к. во многие ф-ции добавился номер сокета.
 * Ф-цию processAction выделил в отдельный .cpp-файл с таким же названием.(много времени убил на борьбу с duplicate definition, #include ...)
 * ...
 * Острая нехватка документации... Долго всё вспоминается. Это при том, что сам весь код писал :)
 *
 * Будем разделять код на случаи, когда нужно знать текущий "rc" экшена и когда - не нужно.
 * ЧТобы запускать манипулятор - не нужно, "выполнять" и точка.
 * Чтобы передать инфу в ЦУП  - нужно.
 * Т.к. экшены в mobman и hiwonder различаются, то решил не делать для мобмана отдельный jsoninfo.cpp, а добавил к уже имеющимся от hiwonder.
 * Добавлены экшены :
 * - "ready"
 * - "parking"
 * - "get_box"
 *
 * Сразу забыл их добавить в jsnObjArray, поэтому надо отдельно потестить.
 *
 * //++++++++++++++++++++++++++++++++++++++++++++++++
 * 04.04.2022
 * Были внесены изменения в работу CV, т.е. переделан сам сервер, принимающий запросы для CV. Это сервер - внешний по отношению к нам. Мы только tcp-запросы туда посылаем.
 * Так вот в связи с изменениями теперь приходит ответ другой. Раньше парсинг ответа начинался на слове "distance" и заканчивался на символе "}", т.к. "distance" был последний параметр.
 * А теперь это уже не последний и там идет ",". Вот чтобы не зависеть от последующих изменений теперь берем только json-значения поля "distance".
 * Однако ранее (24.01.2022 ) это было сделано в проекте gui-client, затем протестировано 26.01.2022 уже в текущем проекте... и почему-то закоментировано.
 * ВОзможно потому, что " НО... пока это работает т.к. заранее известны поля и их типы значений. НА универсальность претендовать не может.
 ...
 *
 * Сейчас это будет приоритетная задача.
 * - Получить список ключей
 * - Если нет ключа "distance" - сообщение в лог об ошибке.
 * - Найти значение ключа "distance"
 *
 * //++++++++++++++++++++++++++++++++++++++++++++++++++
 * 19.03.2022
 * Добавил точки для маленького кубика.
 * Подправил код, т.к. теперь другие имена массивов.
 * //++++++++++++++++++++++++++++++++++++++++++++++++++
 * 14.03.2022
 * Теперь массив Servos инициализируется значениями, соответствующими позиции mob_moving_position а было 93,93...
 *
 * //++++++++++++++++++++++++++++++++++++++++++++++++++
 * 11.03.2022
 * В bash мы привыкли, что если exit-code == 0 - это ОК, т.е. все хорошо.
 * А в qt 0 означает FALSE, блин...
 *
 * Поправил открытие SerialPorta. Если не открывается порт ACM0, то пробуем ACM1.
 * Пока не получилось протестировать. По закону подлости все время ACM0 определялся.
 *
 * //++++++++++++++++++++++++++++++++++++++++++++++++++
 * 09.03.2022
 * В конструкторе HiWonder::HiWonder() приходится заново прописывать имена экшенов. А как бы так сделать, чтобы брать их из списка const QList<QString> tcpCommand
 * который вроде сам по себе ???
 * После выполнения команды "put_box" следует вернуть манипулятор в положение "formoving".
 * Так вот при этом хват снова захватывает кубик и утаскивает с собой.
 * Так прописан порядок движения приводов в arduino.
 * Чтобы на заморачиваться с перепрошивкой arduino ( а тогда надо заново прописывать движения "туда", "обратно"...)
 * Добавил внутрь ProcessAction для команды put_box "поднятие" 4-го привода (хват - 1й привод) после "разжимания" или сброса кубика.
 * Т.е. команда "put_box" - это 4 последовательных команды :
 * 1. Манипулятор в позицию "mob_put_23"
 * 2. Открыть хват.
 * 3. Поднять крайний привод, чтобы не схватить снова кубик при следующем движении.
 * 4. Манипулятор в позицию "mob_moving_position" - в этой позиции хват сам закрывается.
 *
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++++++++
 * 06.03.2022
 * Добавил функционал : взятие кубика, перевод манипулятора с кубиком в положение "formoving".
 * Не смог протестировать вывод сообщений, т.к. распознавалка перестала правильно определять дистанцию.
 * Весь добавленный функционал находится в GetBox, последовательно, чтобы все команды были в одном потоке.
 *
 *
 * //+++++++++++++++++++++++++++++++++++++++++++++++++++
 * 02.03.2022
